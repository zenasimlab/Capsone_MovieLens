...
Now we construct predictors and there is further improvement of RMSE.
...
We then plot rmse vs lamdas to select the optimal lamda
...
The RMSE following regularization approach of Movie and User Effect Model had further decreased the RMSE. 

## Regularized with all effects Model-added genres and year effects

Finally we utilize the regularization approach with inclusion of genres and year effects. 

```{r regularized_with_all_effects,echo=TRUE}
...
The models from most accurate to least accurate are as follows; Regularized Movie,User,Year and Genre Effect Model, Regularized Movie and User Effects Model; Movie and User Effects Model; Movie Effect Model; and Simple Average Model.
...
The regularization approach is the optimal model for prediction with lower RMSE value. The final RMSE is 0.863 with an improvement over 13.3% compared to the baseline model. 
